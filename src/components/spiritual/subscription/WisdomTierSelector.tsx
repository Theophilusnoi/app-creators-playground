...
import { Check, Star, Crown, Mountain, Flame, Droplet, Clock } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {WISDOM_TIERS.map((tier) => {
          const isComingSoon = tier.comingSoon || false;
          ...
          return (
            <Card
              ...
              } backdrop-blur-sm ${isComingSoon ? 'opacity-75' : ''}`}
            >
              {isComingSoon && (
              ...
              <CardHeader className="pb-4">
                <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getElementGradient(tier.element)} flex items-center justify-center mb-4`}>
                  {isComingSoon ? <Clock className="w-6 h-6" /> : getElementIcon(tier.element)}
                </div>
                ...
                <div className="text-center">
                  ...
                  {billingCycle === 'yearly' && !isComingSoon && (
              ...
              <CardContent className="space-y-4">
                ...
                <Button
                  onClick={() => !isComingSoon && onSelectTier(tier)}
                  disabled={currentTier === tier.id || isComingSoon}
                  className={`w-full ${
                    currentTier === tier.id
      ...
      <div className="text-center text-purple-300 text-sm">
        ...
        <p className="mt-2 text-xs text-gray-400">
          Higher tiers coming soon - start your journey with Earth Keeper or Water Bearer
        </p>
      </div>
    </div>
  );
